...
UART_to_CPU: # Tratamento de interrupção de LEITURA da UART
	lbu $s4, 8($s2)        # Lê o caractere escrito na UART A e passa para o registrador $s4
...
MyMain:
	...
	la $s1, mensagemRecebida     # $s1 é o ponteiro de mensagemRecebida
        ...
        li $s4, 1 # Carrega valor qualquer no registrador
...
AguardaRecepcao:
	beq $s4, $zero, SaltoMyMain # Caso o caractére lido seja igual a 0 (fim da frase), desvia para o envio da mensagem de sisB para sisA
	nop
	j AguardaRecepcao
				
SaltoMyMain: 
	lbu $s3, 0($s0)  # Carrega a letra da mensagem em $s3
       	beq $s3, $zero, Fim # Caso o caractére lido seja igual a 0 (fim da frase), desvia para o fim do programa
       	sb $s3, 4($s2)  # Armazena a letra na UART B
        nop
...
mensagemTransmitida:            .asciiz "Teste compreendido!" 
